...
// === Backend API config ===
const API_BASE = ""; // ถ้า frontend กับ backend อยู่โดเมนเดียวกัน ปล่อยว่างได้

async function loadPrizesFromServer() {
  try {
    const res = await fetch(API_BASE + "/api/prizes");
    if (!res.ok) {
      console.error("โหลดรางวัลจาก server ไม่สำเร็จ", res.status);
      return null;
    }
    const data = await res.json();
    return data.prizes || [];
  } catch (err) {
    console.error("เกิดข้อผิดพลาดในการโหลดรางวัลจาก server:", err);
    return null;
  }
}

async function savePrizesToServer() {
  try {
    const res = await fetch(API_BASE + "/api/prizes", {
      method: "POST",
    ...
    if (!res.ok) {
      console.error("เซฟรางวัลไป server ไม่สำเร็จ", res.status);
    }
  } catch (err) {
    console.error("เกิดข้อผิดพลาดในการเซฟรางวัลไป server:", err);
  }
}

// === Storage wrapper (แทน LocalStorage เดิม) ===
async function loadPrizes() {
  const saved = await loadPrizesFromServer();

  if (saved && Array.isArray(saved) && saved.length > 0) {
    prizes = saved.map((prize) => ({
      ...prize,
...
async function addPrize() {
  ...
  if (prizes.length >= 30) {
    alert("สามารถเพิ่มรางวัลได้สูงสุด 30 รายการ!");
    return;
  }
...
function generateRandomColor() {
  const colors = [
    ...
    "#245BFF", // deep blue
    "#5C2DFF", // violet
    "#8A2EFF", // bright violet
    "#B33CFF", // purple neon
    "#FF4FD8", // magenta neon
    "#FF6EC7", // pink neon
    "#FF9B00", // orange accent
